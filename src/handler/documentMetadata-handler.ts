...
export async function findDocumentByLoginId(event: APIGatewayProxyEvent): Promise<any> {
...
export async function documentData(event: APIGatewayProxyEvent): Promise<any> {
    ...
    case ("GET"): {
      ...
      if (event.pathParameters !== null && event.pathParameters !== undefined) {
        if (event.pathParameters.login_id) {
          ...
          const result = await fetchDocumentDataCall(login_id);
          if (result) {
            return response.ok(result);
          }
          else {
